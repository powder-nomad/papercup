...
The voice stack lives in `@papercup/voice-stack`. Three Node wrappers + two Python sidecars + three ONNX models.
...
**Model:** Configurable via `WHISPER_MODEL` env var. Defaults to `base.en`.
...
## TTS: Kokoro (via kokoro-onnx sidecar)

**Where:**
- Pluggable interface: `packages/voice-stack/src/tts/index.ts`
- Kokoro impl: `packages/voice-stack/src/tts/kokoro.ts`
- Python sidecar: `packages/voice-stack/sidecar/tts_kokoro.py`

**Models:** `kokoro-v1.0.onnx` (~325 MB) + `kokoro-voices-v1.0.bin` (~28 MB)

**Native sample rate:** 24 kHz mono. The bot upsamples to 48 kHz stereo for Discord via `mono24kS16ToStereo48kS16`.

**Wire protocol** (mixed binary + line-buffered text):
...
**Voices:** 54 loaded by default. American English (`af_*`, `am_*`), British (`bf_*`, `bm_*`), Japanese (`jf_*`, `jm_*`), Mandarin (`zf_*`, `zm_*`), and others. **No Korean.**

```ts
import { createTts } from "@papercup/voice-stack/tts";

const tts = createTts(process.env.TTS_ENGINE ?? "kokoro");
await tts.start();
const { pcm, sampleRate, durationMs } = await tts.synthesize("hello world");
```
